	...
	if (totalRate==0) {
       ...
       // }
        ...
        if (cSS.executionNumber%200==0) {
            ...
            << capK["R4"]["Kstep1"]*capK["R4"]["Kstep2"]*cSS.numMoieties["cEp"]*cSS.numMoieties["Emp"]*1.0/(V*Na)*1.0/(V*Na)/denom << "  "
            << capK["R5"]["Kstep1"]*capK["R5"]["Kstep2"]*cSS.numMoieties["cE"]*cSS.numMoieties["Ep"]*1.0/(V*Na)*1.0/(V*Na)/denom << "  "
            << capK["R6"]["Kstep1"]*capK["R6"]["Kstep2"]*cSS.numMoieties["cE"]*cSS.numMoieties["Emp"]*1.0/(V*Na)*1.0/(V*Na)/denom << "  "
            << capK["R7"]["Kstep1"]*capK["R7"]["Kstep2"]*cSS.numMoieties["cE"]*cSS.numMoieties["Em"]*1.0/(V*Na)*1.0/(V*Na)/denom << "  "
