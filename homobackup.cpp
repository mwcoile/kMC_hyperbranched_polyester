...
double approxChainLengthDependentk(int i) {
    ...
    return 1.0; //Kij_divided_by_k_int; //1.0
}
...
int main() {
    ...
    unsigned int seed = rand_dev(); // currently busted with seed 961071223 // try 1386656568 //works with 1124697860; // for debugging, can set this to a constant value if desired, e.g., 1086615307; 
    ...
    while (totalRate>0 && simTime<60*60*24*2) {
        ...
        //}
        //cSS.numMoietiesByLength
        for (int i=0;i<cSS.chainPool.size(); ++i) {
            // honestly, numMoieties on monomers doesn't matter for this... so we could just ignore.
        ...
        std::vector<std::map<std::string,long>> regCumSum = {myConditions.simulationSize+1,std::map<std::string,long>()};
        // initialize normalizedCumSum with empty maps of the length of the simulation size (?does this actually need to happen)

        std::vector<std::map<std::string,double>> normalizedCumSum = {myConditions.simulationSize+1,std::map<std::string,double>()};
        for (long i=1; i<myConditions.simulationSize+1;++i) {
        ...
        for (auto iter = cSS.numMoieties.begin(); iter!=cSS.numMoieties.end(); ++iter) {
            ...
            for (int i=0;i<cSS.numMoietiesByLength.size();++i) {
                confirmCountsMatch[iter->first]+=cSS.numMoietiesByLength[i][iter->first];
            }
        }
        ...
        +capK["R1"]["Kprimestep4"]*cSS.links[P1]*1.0/(V*Na)
        +capK["R2"]["Kprimestep4"]*cSS.links[P2]*1.0/(V*Na)
        +capK["R3"]["Kprimestep4"]*cSS.links[P3]*1.0/(V*Na)
        +capK["R4"]["Kprimestep4"]*cSS.links[P4]*1.0/(V*Na)
        ...
        +capK["R9"]["Kprimestep4"]*cSS.links[P9]*1.0/(V*Na)
	...
	std::vector<double> ratesDebugging(20,0);
        ...
        for (int i=0; i<reactionList.size(); ++i) {
		    ...
		    double kf=rateConstants[R_i]["kf"];
                    ...
                    for (long length=1; length<=myConditions.simulationSize;++length) {
                        double kf=rateConstants[R_i]["kf"];
                        double kr=rateConstants[R_i]["kr"];
                        double kfprime = capK[R_i]["Kstep1"]*kf;
                        double cfprime = kfprime/(V*Na); // kf is trimolecular for these linking reactions: catalyst+2 reactants
                        ...
                        if (reactionList[i].reactants.size()==2) {
                            ...
                            if (moiety1==moiety2 && cSS.numMoieties[moiety1]==1) {
                                ...
                                double rateFDividedByStar=cfprime*(normalizedCumSum[length][moiety1]*cSS.numMoietiesByLength[length][moiety2] +
                                (cSS.numMoieties[moiety1]-regCumSum[length][moiety1])*cSS.numMoietiesByLength[length][moiety2]*transport_factor[length]); // need num of each functional group overall tracked.
	...
	if (totalRate==0) {
       ...
       // }
        ...
        //if (mu<rate.size()) {
        updateSystemState(cSS, reactionList[chosenRxn.mu], r3, r4, myOptions,timers,chosenRxn,reactionList,chosenRxn.length,ratesByLength[chosenRxn.length][chosenRxn.chosenR_i],transport_factor);
        ...
        if ((cSS.executionNumber%5==0) && myOptions.Changxia) {
        ...
        //calculateDPrw(cSS,conv);
        //conv << simTime << "     " << over_x << "  " << rate[1]/(1.0*myConditions.simulationSize) << std::endl;
