...
struct UpdateNMBLInput {
	std::string rxnType = "";
	long length1 = 0;
	long length2 = 0;
	long length3 = 0;
	std::map<std::string,signed long> funs1;
	std::map<std::string,signed long> funs2;
	std::map<std::string,signed long> funs3;
};
//std::vector<std::map<std::string,signed long>>
void updateNumMoietiesByLength(std::vector<std::map<std::string,signed long>>& numMoietiesByLength,UpdateNMBLInput input) {
	// this function takes as input 
	// rxnType = "linking" OR "breaking"
	// if rxnType is linking
	// funs1: funs on chain 1
	// funs2: funs on chain 2
	// funs3: funs on final linked chain
	// if rxnType is breaking
	// funs1: funs on original chain
	// funs2: funs on chain fragment 1
	// funs3: funs on chain fragment 2
	// 
	// numMoietiesByLength is what we're updating
	//
        // this function is specific to Changxia's hyperbranched polyester system 
	// synthesized from lactones	
        if (input.rxnType=="linking") {
                if (input.length1>1) {
			// now subtract these from grpsByLength
                	for (auto iter= input.funs1.begin(); iter!=input.funs1.end(); ++iter) {
                        	numMoietiesByLength[input.length1][iter->first]-=iter->second;
                            if (numMoietiesByLength[input.length1][iter->first]==0){
                                numMoietiesByLength[input.length1].erase(iter->first);
                            }
                	}
		}
                if (input.length2>1) {
			// now subtract these from grpsByLength
                	for (auto iter= input.funs2.begin(); iter!=input.funs2.end(); ++iter) {
                        	numMoietiesByLength[input.length2][iter->first]-=iter->second;
                            if (numMoietiesByLength[input.length2][iter->first]==0){
                                numMoietiesByLength[input.length2].erase(iter->first);
                            }
                	}
		}
		if (input.length3>1) {
			// now add these to grpsByLength
                	for (auto iter= input.funs3.begin(); iter!=input.funs3.end(); ++iter) {
                        	numMoietiesByLength[input.length3][iter->first]+=iter->second;
                            if (numMoietiesByLength[input.length3][iter->first]==0){
                                numMoietiesByLength[input.length3].erase(iter->first);
                            }
                	}
		} 
		else {
			throw std::runtime_error("this should not happenn"); 
		}
	}
	else if (input.rxnType == "breaking") {
		// now subtract these from grpsByLength
		for (auto iter= input.funs1.begin(); iter!=input.funs1.end(); ++iter) {
			numMoietiesByLength[input.length1][iter->first]-=iter->second;
                            if (numMoietiesByLength[input.length1][iter->first]==0){
		...
		throw std::runtime_error("unexpected input to updateGrpsbyLength function");
	}
	return;// numMoietiesByLength;
...
Vertex findVertexOnChain(std::string reactant, const Chain& selectedChain, int grpCounter,double whichGrp) {
    //grpCounter-=cSS.chainPool[i].funs.at(groupName);
                    
    auto vpair = boost::vertices(selectedChain.polymer);
    for (auto iter = vpair.first; iter!=vpair.second; iter++) {
        // add the funs on the given monomer to the counter
        std::map<std::string,signed long>::const_iterator funsOnMonomerCount=selectedChain.polymer[*iter].funs.find(reactant);
        if (funsOnMonomerCount!=selectedChain.polymer[*iter].funs.end()) {
...
GroupLocation linearSearchForParticipatingGroupByLength(const SystemVariables &cSS,const double &randomNumber, std::string groupName,int length) {
    // find and return a particular group to react
    // this function is designed to take as input a group name, a random number, the chain length on which to find the group, and the current
    // system state and select (at random) a group with one of these lengths.
    GroupLocation chosenGroup;
    double whichGrp = randomNumber*cSS.numMoietiesByLength[length].at(groupName);
    int grpCounter=0;
    ...
    // chain+chain reactions don't involve monomers
    ...
    for (int i=0; i<cSS.chainPool.size();i++) {
        ...
        if (boost::num_vertices(cSS.chainPool[i].polymer)!=length) {
            continue;
        }
        else {
            ...
            if (grpCounter>whichGrp) {
                ...
                grpCounter-=cSS.chainPool[i].funs.at(groupName);
                //chosenGroup.whichVertex = findVertexOnChain(groupName, cSS.chainPool[i],  grpCounter, whichGrp);
...
void updateLinks(std::map<std::string, signed long> &links,Chain &myChain,const std::string &oldVertName, const std::string &newVertName, std::pair<edgeIter,edgeIter> edgs, const Edge &newEdge) { 
    ...
    if (isOutEdges) {
        while (edgs.first!=edgs.second) {
            ...
            std::string linkstgt="";
            Vertex linkstgtVertex=target(*edgs.first,myChain.polymer);
    ...
    // if in edges, target remains constant
        ...
        while (edgs.first!=edgs.second) {
            ...
            std::string linkssrc="";
            Vertex linkssrcVertex=source(*edgs.first,myChain.polymer);
...
struct lengthBasedGroupLocations {
    GroupLocation grp1;
    GroupLocation grp2;
};


Vertex findVertexOnChainForLengthDependentRxn(std::string reactant, const Chain& selectedChain, int grpCounter,double whichGrp) {
    //grpCounter-=cSS.chainPool[i].funs.at(groupName);
...
lengthBasedGroupLocations lengthDependentSearchForParticipatingGroups(SystemVariables cSS,double r3, double r4, std::string reactant1,std::string reactant2, int length,double ratesByLength_lengthRi,std::vector<double> transport_factor,double ksteptimesstar_muminus4) {
    lengthBasedGroupLocations selectedGroups;
    // First select group 1 (which is always a cE). Need ratesByLength[length][R_i] 
    double sumRates = 0;
    double rateContributionFromThisChain=0;
    for (int i=0;i<cSS.chainPool.size();++i){
        // every chain should have one cE unless looping is allowed
        // this is specific to changxia's hyperbranched polyester system.
        int chainSize = boost::num_vertices(cSS.chainPool[i].polymer);
        if (chainSize<length) {
            rateContributionFromThisChain=  ksteptimesstar_muminus4 * 1.0 * transport_factor[chainSize] * cSS.numMoietiesByLength[length][reactant2];
        }
        else {
            rateContributionFromThisChain = ksteptimesstar_muminus4 * 1.0 * transport_factor[length] * cSS.numMoietiesByLength[length][reactant2];
        }
        sumRates+=rateContributionFromThisChain;
        if (sumRates>ratesByLength_lengthRi*r3) {
            selectedGroups.grp1.whichChain=i;
            // i way over thought this. Thre is one cE.. just find it
            //auto vpair = ;
            for (auto vpair = boost::vertices(cSS.chainPool[i].polymer); vpair.first!=vpair.second; vpair.first++) { 
                if (getChainUnitName(cSS.chainPool[i].polymer[*vpair.first])=="cE") {
                    selectedGroups.grp1.whichVertex = *(vpair.first);
                    //selectedGroups.grp1.whichChain = i;
                    goto grouptwoselection;
                }
            }
...
grouptwoselection:
    // Now select group 2 (which is never a cE (or cEp))
    selectedGroups.grp2 = linearSearchForParticipatingGroupByLength(cSS,r4,reactant2, length);
    if (selectedGroups.grp1.whichChain==-1) {
        throw std::runtime_error("holy moses this is not correct!");
    }
...
void linkingRxn(SystemVariables &cSS, Reaction rxn,double r3,double r4, ModelOptions myOptions,std::map<std::string,DoubleSec>& timers,chosenReactionChannel chosenRxn, std::vector<Reaction> reactionList,int length, double ratesByLength_lengthRi,std::vector<double> transport_factor,std::vector<double> ksteptimesstar) {
    ...
    GroupLocation selectedReactant1;
    GroupLocation selectedReactant2;    
    if (myOptions.trackAllLinks == false ) {
    ...
    if (chosenRxn.isLengthDependent==false) {
        selectedReactant1 = linearSearchForParticipatingGroup(cSS,r3,reactant1,myOptions.trackFullStructureAsGraph);
    ...
    if (selectedReactant1.whichChain>-1) {
        inputNMBL.funs1=cSS.chainPool[selectedReactant1.whichChain].funs;	
        inputNMBL.length1=boost::num_vertices(cSS.chainPool[selectedReactant1.whichChain].polymer);
    }
    else {
        inputNMBL.length1=1;
    }
    if (selectedReactant2.whichChain>-1) {
        inputNMBL.funs2=cSS.chainPool[selectedReactant2.whichChain].funs;
        inputNMBL.length2=boost::num_vertices(cSS.chainPool[selectedReactant2.whichChain].polymer);
    }
    else {
        inputNMBL.length2=1;
    }
    ...
    if (whichChain1==-1 && whichChain2==-1) {
        ...
        */
        ...
        // update numMoietiesByLength
        
        inputNMBL.length3=boost::num_vertices(newChain.polymer); // should always be 2
        inputNMBL.funs3 = newChain.funs; // should always be cE=1 and Emp=1 and all else 0.
    ...
    // if one monomer and one chain are chosen to react
        ...
        // update numMoietiesByLength trackers
        inputNMBL.length3=boost::num_vertices(cSS.chainPool[chainSelected].polymer);
        inputNMBL.funs3=cSS.chainPool[chainSelected].funs;
    ...
    // else, if two chains are chosen to react, execute the following code
        ...
        if (whichChain1!=whichChain2 || myOptions.loopingSeparateChannel==false) {
            ...
            // update numMoietiesByLength trackers
            inputNMBL.length3=boost::num_vertices(cSS.chainPool[whichChain1].polymer);
            inputNMBL.funs3=cSS.chainPool[whichChain1].funs;
    ...
    // update numMoietiesByLength
    updateNumMoietiesByLength(cSS.numMoietiesByLength,inputNMBL);
    
    long chainDP = inputNMBL.length3;
    if (chainDP>cSS.maxChainSize) {
        cSS.maxChainSize=chainDP;
    }
    //cSS.numMoietiesByLength = updateNumMoietiesByLength(cSS.numMoietiesByLength,inputNMBL);
    return;
}
...
void separateFragments(std::vector<Chain> &chainPool,long chainIndex,Connectivity myConnect) {
    ...
    boost::graph_traits<Graph>::vertex_iterator startVertex=(boost::vertices(chainPool[chainIndex].polymer).first);
    boost::graph_traits<Graph>::vertex_iterator startVertexd=(boost::vertices(d).first);
    ...
    for (int i = 0; i < totalVertices; ++i) {
        ...
        if (myConnect.component[i] != 0) {
            int index = i-countDeletedEdges; // calculate index in the old chain
            // update chain's functional group count at the end...
            //Vertex vertexToClear=std::advance()
            
            boost::clear_vertex( *(std::next(startVertex,i-countDeletedEdges)),chainPool[chainIndex].polymer);
            boost::remove_vertex(*std::next(startVertex,i-countDeletedEdges),chainPool[chainIndex].polymer);
            countDeletedEdges++;
        }
        else {
            int index=i-countDeletedEdgesNewGraph; // calculate index of the new chain
            // calculate the effect of deleting the particular monomer
            for (auto itfun = d[*std::next(startVertexd,index)].funs.begin(); itfun != d[*std::next(startVertexd,index)].funs.end(); itfun++ ) {
                newFragment.funs[itfun->first]-=itfun->second;
            } 
            // update chain weight
            newFragment.weight-=d[*std::next(startVertexd,index)].weight;
            boost::clear_vertex(*std::next(startVertex,i-countDeletedEdgesNewGraph),d); // delete all edges associated with the vertex
            boost::remove_vertex(*std::next(startVertex,i-countDeletedEdgesNewGraph),d);
            countDeletedEdgesNewGraph++;
        }
    }
...
void adjacentBreakingRxn(SystemVariables &cSS, Reaction rxn,double r3,double r4,ModelOptions myOptions,std::map<std::string,DoubleSec>& timers) {
    ...
    // declare our numMoietiesByLength trackers
    UpdateNMBLInput inputNMBL;
    inputNMBL.rxnType="breaking";
        ...
        if (linkCounter>whichLink) {
            ...
            while (edgs.first!=edgs.second) {
                ...
                if (linkCounter>whichLink) {
                    ...
                    srcBust=src;
                    tgtBust=tgt;
                    inputNMBL.length1=boost::num_vertices(cSS.chainPool[polymerToBust].polymer);
                    inputNMBL.funs1=cSS.chainPool[polymerToBust].funs;
                    breakOutOfLoop=true;
                    break;
                }
                edgs.first++;
    ...
    // create undirected graph to check for connectivity
    UndirectedGraph uG;
    DoubleTimePoint startTimingConnectedComponents=Time::now(); //get the time at this instant
    copy_graph(d, uG); // there may be a more efficient way of creating an undirected graph from a directed graph
    ...
    DoubleTimePoint stopTimingConnectedComponents=Time::now(); //get the time at this instant
    timers["connectedcomponents"]+=stopTimingConnectedComponents-startTimingConnectedComponents;
    DoubleTimePoint startTimingTheSplit = Time::now(); //get the time at this instant 
    boost::graph_traits<Graph>::vertex_iterator startVertex= (boost::vertices(cSS.chainPool[polymerToBust].polymer).first);
    boost::graph_traits<Graph>::vertex_iterator startVertexd=(boost::vertices(d).first);
    ...
    if (num_components==1) {
        ...
        boost::clear_vertex(*std::next(startVertex,possibleMonomerIndex1),cSS.chainPool[polymerToBust].polymer);
        boost::remove_vertex(*std::next(startVertex,possibleMonomerIndex1),cSS.chainPool[polymerToBust].polymer);
        ...
        // update NMBL trackers
        inputNMBL.length2=1; 
        // check if the second is a monomer, i.e., if the splitting reaction was a dimer into two monomers
        if (component.size()==2) {
            // update NMBL trackers
            inputNMBL.length3=1;
            //increment monomer count
            ...
            // set the NMBL trackers for chain 3 accordingly
            inputNMBL.length3=inputNMBL.length1-1;
            inputNMBL.funs3=cSS.chainPool[polymerToBust].funs;
        }
        ...
        // it's just a monomer that is coming off! Just erase the one...
        // delete the one vertex from the old graph!
        boost::clear_vertex(*std::next(startVertex,possibleMonomerIndex2),cSS.chainPool[polymerToBust].polymer);
        boost::remove_vertex(*std::next(startVertex,possibleMonomerIndex2),cSS.chainPool[polymerToBust].polymer);
        ...
        // update NMBL trackers
        inputNMBL.length3=1;
        inputNMBL.length2=inputNMBL.length1-1;
        inputNMBL.funs2=cSS.chainPool[polymerToBust].funs;
    }
    else {
        // remove the stuff at the "end" of the old chain
        // most likely, the start of the old graph is "component 0" but verify this...
        ...
        for (int i = 0; i < totalVertices; ++i) {
            ...
            if (component[i] != 0) {
                ...
                boost::clear_vertex(*std::next(startVertex,i-countDeletedEdges),cSS.chainPool[polymerToBust].polymer);
                boost::remove_vertex(*std::next(startVertex,i-countDeletedEdges),cSS.chainPool[polymerToBust].polymer);
                countDeletedEdges++;
            }
            else {
                int index=i-countDeletedEdgesNewGraph; // calculate index of the new chain
                ...
                if (myOptions.trackAllLinks) {
                    ...
                    for (auto out_edgs = boost::out_edges(*std::next(startVertexd,index), d); out_edgs.first!=out_edgs.second; ++out_edgs.first) {
                        std::string linkName = getLinkName(d,*out_edgs.first);
                        newFragment.linkTotals[linkName]--;
                    }

                    //check in edges -- is this redundant ?
                    for (auto in_edgs = boost::in_edges(*std::next(startVertexd,index), d); in_edgs.first!=in_edgs.second; ++in_edgs.first) {
                ...
                // update chain weight
                newFragment.weight-=d[*std::next(startVertexd,index)].weight;
                boost::clear_vertex(*std::next(startVertexd,i-countDeletedEdgesNewGraph),d); // delete all edges associated with the vertex
                boost::remove_vertex(*std::next(startVertexd,i-countDeletedEdgesNewGraph),d);
                countDeletedEdgesNewGraph++;
            }
        }
        DoubleTimePoint stopTimingTheSplit = Time::now(); //get the time at this instant 
        timers["split"] += (stopTimingTheSplit - startTimingTheSplit);
        ...
        // the below code should never find monomers, because they have been checked for in "countComp1" and "countComp2" already.
        ...
        if (checkMonomer.first==false) {      
            ...
            std::cout << "did not expect this to happen" << std::endl;
            // grab monomer name
            cSS.numMonomers[checkMonomer.second]++;
        }
        // check if the old chain is a monomer -- if so, add it to the monomer count (this should be caught earlier)
        std::pair<bool,std::string> checkOldChain= isMonomer(cSS.chainPool[polymerToBust]);
        if (checkOldChain.first==true) {      
            cSS.chainPool.erase(cSS.chainPool.begin()+polymerToBust);
            cSS.numMonomers[checkMonomer.second]++;
            std::cout << "did not expect this to happen either" << std::endl;
        }
        ...
        // update the NMBL trackers
        inputNMBL.length2=boost::num_vertices(newFragment.polymer);
        inputNMBL.funs2=newFragment.funs;
        inputNMBL.length3=boost::num_vertices(cSS.chainPool[polymerToBust].polymer);
        inputNMBL.funs3=cSS.chainPool[polymerToBust].funs;
    }
    ...
    // the below assumes that no loops have formed (which is what I am assuming
    // for Changxia's system
     
    long chainDP = inputNMBL.length1;
    if (chainDP==cSS.maxChainSize) {
        long biggestChain=1;
        for (int i=0;i<cSS.chainPool.size();++i) {
            long chainSize=boost::num_vertices(cSS.chainPool[i].polymer);
            if (chainSize>biggestChain) {
                biggestChain=chainSize;
            }
        }
        cSS.maxChainSize=biggestChain;
    }
    return;
}
...
void updateSystemState(SystemVariables &cSS, Reaction rxn,double r3,double r4,ModelOptions myOptions, std::map<std::string,DoubleSec>& timers ,chosenReactionChannel chosenRxn, std::vector<Reaction> reactionList,int length, double ratesByLength_lengthRi,std::vector<double> transport_factor,std::vector<double> ksteptimesstar) {
	...
	bool couldstophere=true;
    }
    if (chosenRxn.isLengthDependent) {
	    bool heywhatsup=true;
    }
    if (cSS.executionNumber>=3000) {
	    bool couldalsostophere=true;
    }
    ...
    if (rxn.reactionType=="looping") {
        ...
        throw std::runtime_error("unexpected with changxia's system");
        // DoubleTimePoint stop = Time::now(); //get the time at this instant 
